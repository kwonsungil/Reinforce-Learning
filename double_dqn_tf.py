...
from config import config_double_dqn as config
...
class DQN(object):
    def __init__(self, is_train, game):
        ...
        with self.graph.as_default():
            ...
            self.Q_main = self.create_network('main_double_dqn')
            self.Q_target = self.create_network('target_double_dqn')
            ...
            if self.is_train:
                ...
                tf.summary.scalar('double_dqn/rewards', tf.reduce_sum(self.rewards))
                tf.summary.scalar("double_dqn/loss", self.cost)
                tf.summary.scalar("double_dqn/lr", self.learning_rate)
            ...
            for episode in range(1, config.MAX_EPISODES):
                ...
                if episode % 10 == 0:
                    for _ in range(50):
                        ...
                        Q_main = self.sess.run(self.Q_main, feed_dict={self.states: next_states})
                        Q_target = self.sess.run(self.Q_target, feed_dict={self.states: next_states})
                        ...
                        for i in range(0, config.BATCH_SIZE):
                            ...
                            if done:
                                ...
                                # main network에서 가장 큰 Q를 얻을 수 있는 action
                                a = np.argmax(Q_main[i])
                                # 그 action을 했을 경우 target network에서 얻을수 있는 Q 값
                                y_targets.append(rewards[i] + config.GAMMA * Q_target[i][a])
