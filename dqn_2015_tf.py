...
from config import config_dqn_2015 as config
...
class DQN(object):
    def __init__(self, is_train, game):
        ...
        with self.graph.as_default():
            ...
            self.Q_main = self.create_network('main_2015')
            self.Q_target = self.create_network('target_2015')
                    ...
                    for _ in range(50):
                        ...
                        for i in range(0, config.BATCH_SIZE):
                            ...
                            if done:
                                ...
                                y_targets.append(rewards[i] + config.GAMMA * np.max(Q_target[i]))
